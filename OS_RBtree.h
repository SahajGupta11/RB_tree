/*
    ...
    Additionally supports order statistic.
        1. order_of_key(key):   determines the no. of elements
                                strictly smaller than a given key
        2. find_by_order(k):    returns an iterator to kth element 
                                (0-based indexing)
*/
#ifndef OS_RB_TREE_H 
#define OS_RB_TREE_H 
...
// Order statistic
#define order_of_key(T, x)                              /* order_of_key(&T, iterator) or order_of_key(&T, key) */ \
    _Generic((x),                   \
        iterator: order_of_key_it,  \
        key_type: order_of_key_k    \
    )(T, x)
int order_of_key_it(Rb_tree *T, iterator it);           // find order statistic in O(logn)
int order_of_key_k(Rb_tree *T, key_type key);           // returns number of elements strictly smaller than key
iterator find_by_order(Rb_tree *T, int k);              // returns iterator to kth element (0-based) in O(logn)
